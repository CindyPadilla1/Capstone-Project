...
async function run() {
    ...
    // Insert survey_trigger
    await pool.query(
        `INSERT INTO survey_trigger
           (schedule_id, user1_id, user2_id, trigger_at, sent, created_at)
         VALUES ($1, $2, $3, NOW() - INTERVAL '30 minutes', false, NOW())`,
        [liveScheduleId, u1, u2]
    );
    console.log(`  ✓ survey_trigger inserted`);
    ...
    console.log('\n' + '═'.repeat(62));
    console.log('  ✅  FEATURE 2 DEMO READY');
    console.log('═'.repeat(62));
    console.log(`  Finley   user_id=${finleyId}   finley@test.com   password123`);
    console.log(`  Morgan   user_id=${morganId}   morgan@test.com   password123`);
    console.log(`  match_id=${matchId}`);
    console.log(`  live schedule_id=${liveScheduleId}`);
    console.log('');
    console.log('  DEMO FLOW:');
    console.log('  1. Log into app as Morgan');
    console.log('  2. Matching page — find Finley → "New User", no shields');
    console.log('  3. Click bell → notification already there');
    console.log('  4. Click "Complete check-in" → fill out form live');
    console.log('     felt_safe = No         → -5');
    console.log('     boundaries = No        → -4');
    console.log('     felt_pressured = Yes   → -3');
    console.log('     total delta            → -12');
    console.log('  5. Submit → go back to matching');
    console.log('  6. Finley now shows shields + "Limited Trust"');
    console.log('  7. pgAdmin proof:');
    console.log(`     SELECT internal_score, public_trust_rating`);
    console.log(`     FROM trust_score WHERE user_id = ${finleyId};`);
    console.log('═'.repeat(62));
